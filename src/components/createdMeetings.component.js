...
const Meeting = props => (
    <tr>
      ...
      <td>
        <Link to={"/edit/"+props.userId+"/"+props.meeting._id}>edit</Link> | <Link to={"/comments/"+props.userId+"/"+props.meeting._id}>comments</Link> | <a href="" onClick={() =>{
              ...
              axios.post("http://localhost:8000/users/removeFromCreated/"+id+"/"+meetingid)
              .then(
              ...
              axios.delete("http://localhost:8000/meetings/" +meetingid , b)
...
export default class CreatedMeetings extends Component{
    constructor(props){
     ...
     componentDidMount() {
      ...
      .then(res => {
        goingto = res.data.listOfCreatedMeetings
      })
      .catch(
      err => console.log(err)
    )
